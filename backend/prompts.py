...
def get_aki_style_prompt(level, topic="", category="grammar", loop_index=0, total_count=1, reference_text=""):
    ...
    3. **選択肢の質と唯一性の保証**:
       - **正解の唯一性**: 選択肢の中に「文脈やニュアンスを変えれば正解になり得るもの」が複数含まれることを絶対に避けること（例：短い一文で「行きます」と「行きましょう」が両方入るなど）。
    ...
    4. **解説の充実**: 解説には「なぜ他の選択肢がその文脈では不適切なのか」を論理的に含めること（「知らんがな」と言われない、納得感のある説明）。
