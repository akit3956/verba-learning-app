...
def get_aki_style_prompt(level, topic="", category="grammar", loop_index=0, total_count=1, reference_text=""):
    ...
    ### 【絶対遵守：作成ルール】
    1. **自然な日本語の徹底**: ネイティブが「絶対にそうは言わない」ような不自然な文言は1文字も入れないこと。
    2. **文法的正確性のガードレール**: 以下のAI特有のミスを絶対に避けること：
    ...
    シチュエーション：{random_inspiration if random_inspiration else "トピックに最適な場面を選択"}
